...
@Component({
  ...
  styleUrls: ['./table-processes.component.scss']
})
export class TableProcessesComponent {
  ...
  paginatedProcesses: Process[] = [];

  @Input() set processes(data: Process[]) {
    this.processesAll = data;
  ...
  sortBy(column: string): void {
    ...
    this.processesAll.sort((a: any, b: any) => {
      const valA = column.includes('.') ? column.split('.').reduce((o, i) => o?.[i], a) : a[column] ?? '';
      const valB = column.includes('.') ? column.split('.').reduce((o, i) => o?.[i], b) : b[column] ?? '';      
  ...
  updatePaginatedProcesses(): void {
    ...
    this.paginatedProcesses = this.processesAll.slice(start, end);
  }
  ...
  get totalPages(): number {
    return Math.ceil(this.processesAll.length / this.pageSize);
  }
